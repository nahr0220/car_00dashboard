...
if not periods.empty:
    ...
    **각 시장의 정의:**
    ...
    # 6. 엑셀 생성 및 다운로드 (6개 시트 완벽 포함)
    if excel_clicked:
        with st.spinner("엑셀 상세 리포트 생성 중..."):
    ...
    fig1 = go.Figure()
    fig1.add_bar(x=g_total["연월라벨"], y=g_total["건수"], name="전체", opacity=0.3, text=g_total["건수"], textposition='outside', texttemplate='<b>%{text:,}</b>', textfont=dict(size=25, color="black"))
    ...
    if not df_ap_m.empty:
        ...
        fig_ap.add_scatter(x=df_ap_m["연월라벨"], y=df_ap_m["AP비중_시각화"], mode="lines+markers+text", text=df_ap_m["AP비중"].round(2).astype(str) + "%", textposition="top center", textfont=dict(size=12, color="red", family="Arial Black"), name="AP 비중 (%)", line=dict(color='red', width=2))
        fig_ap.update_layout(yaxis=dict(tickformat=",d"), margin=dict(t=50))
